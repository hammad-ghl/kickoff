<template>
  <div class="h-full flex flex-col bg-primary overflow-auto">
    <div class="max-w-[1100px] w-full mx-auto px-10 pt-10 pb-6 flex flex-col">
      <!-- Header -->
      <div class="mb-8">
        <router-link to="/ui-libraries" class="text-[13px] text-secondary hover:text-white transition-colors mb-3 inline-flex items-center gap-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
          </svg>
          Back to UI Libraries
        </router-link>
        <h1 class="text-[28px] text-primary font-normal mt-2">Add UI Library</h1>
        <p class="text-[14px] text-secondary mt-2">Connect a GitHub repository containing your UI components</p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="card p-6 space-y-5">
          <h3 class="text-[13px] font-medium text-primary uppercase tracking-wider">Library Info</h3>
          
          <div class="space-y-2">
            <label class="block text-[13px] font-medium text-primary">Library Name *</label>
            <input
              v-model="form.name"
              type="text"
              class="input w-full"
          ...
          <div v-else class="space-y-5">
            ...
            <!-- Component Path -->
            <div v-if="form.repo" class="space-y-2">
              <label class="block text-[13px] font-medium text-primary">Component Path</label>
              <div class="relative">
                <input
                  v-model="pathSearch"
                  type="text"
              ...
              <p class="text-[12px] text-secondary">Leave empty to scan entire repository</p>
            </div>
          </div>
        </div>
